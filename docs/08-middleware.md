...
Let's provide some typing. The `ExpressJS` type declarations (`@types/express`) were installed when we generated a new NestJS project using the CLI at the start of the course.
...
We can port the remaining of the original middleware function as is.
...
Let's start by configuring the class-based middleware from the previous section. Open the application root module (`app.module.ts`). The `AppModule` must now implement the `NestModule` interface from the `@nestjs/common` module.
...
Using the `MiddlewareConsumer` instance we can now configure the middleware. Let's configure the middleware to be configured for all requests, not matter by which HTTP method (`GET`, `POST`...) they are invoked.
...
Although configuring excluded routes is **not** intuitive. A better alternative would be to implement path-restriction logic directly into the middleware or configure it as seperate middleware.

The following thread on NestJS' GitHub account dives deeper into this topic:
